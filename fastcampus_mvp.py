...
class FastCampusCrawler:
    ...
    def _log_progress(self, message, step=None):
        ...
        step_name = self.CRAWLING_STEPS[self.current_step] if self.current_step < len(self.CRAWLING_STEPS) else "알 수 없음"
    ...
    def _extract_main_categories(self, page):
        ...
        self._log_step_complete("메인 카테고리 수집", len(self.data['main_categories']))
    ...
    def _extract_sub_categories(self, page):
        ...
        self._log_step_complete("하위 카테고리 수집", len(self.data['sub_categories']))
    ...
    def _extract_title(self, card, course_url=None):
        ...
        selectors = [
            '.CourseCard_courseCardTitle__1HQgO',  # 정확한 클래스명 (사용자 제공)
            '[data-e2e="display-card"]',  # data-e2e 속성 (사용자 제공)
        ...
        for selector in selectors:
            ...
            if element.count() > 0:
                ...
                if title and len(title) > 3 and not title.isdigit() and not title.endswith('+'):
                    self._log_progress(f"강의 제목 추출 성공: {title[:50]}... (선택자: {selector})")
                    return title
        ...
        for line in lines:
            if len(line) > 3 and not line.isdigit() and not line.startswith(('+', '⚠️')) and not line.endswith('+'):
                self._log_progress(f"강의 제목 추출 성공 (fallback): {line[:50]}...")
                return line
                ...
                for img in images:
                    ...
                    if url:
                        ...
                        if response:
                            ...
                            filename = f"image_{saved_count + 1}{ext}"
        ...
        try:
            ...
            for script in json_scripts:
                try:
                    ...
                    def find_courses(obj, path=""):
                        ...
                        if isinstance(obj, dict):
                            ...
                            for key, value in obj.items():
                                courses.extend(find_courses(value, f"{path}.{key}"))
                                
                        elif isinstance(obj, list):
                            for i, item in enumerate(obj):
                                courses.extend(find_courses(item, f"{path}[{i}]"))
                    ...
                    if courses:
                        self._log_progress(f"JSON에서 {len(courses)}개 강의 제목 추출 완료")
                        return True
    ...
    def _extract_courses(self, page, subcategory, max_courses=3):
        """강의 정보 추출."""
        self._log_progress(f"강의 수집 시작: {subcategory['메인카테고리']} > {subcategory['하위카테고리']}")
        ...
        for selector in card_selectors:
            ...
            if found_cards:
                cards = found_cards
                self._log_progress(f"강의 카드 발견: {len(cards)}개 (선택자: {selector})")
                break
        
        if not cards:
            # 마지막 fallback - 링크 요소들을 카드로 사용
            cards = page.locator(self.SELECTORS['course_link']).all()
            self._log_progress(f"대체 선택자로 강의 카드 발견: {len(cards)}개 (선택자: {self.SELECTORS['course_link']})")
    ...
    def _validate_data(self, data_type, data):
        ...
        validators = {
            "main_categories": lambda d: len(d) == 9 and 
                                       all(item.get('메인카테고리') for item in d),
            "sub_categories": lambda d: len(d) >= 40 and 
                                      all(item.get('하위카테고리') for item in d),
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                for i, category in enumerate(self.data['main_categories'][:2], 1):
                    self._log_progress(f"메인 카테고리 HTML 저장 ({i}/2): {category['메인카테고리']}")
                ...
                for i, category in enumerate(self.data['sub_categories'][:2], 1):
                    self._log_progress(f"하위 카테고리 HTML 저장 ({i}/2): {category['하위카테고리']}")
                ...
                for i, subcategory in enumerate(self.data['sub_categories'][:2], 1):
                    self._log_progress(f"강의 수집 진행 ({i}/2): {subcategory['하위카테고리']}")
                ...
                for i, course in enumerate(self.data['courses'][:2], 1):
                    self._log_progress(f"강의 상세 정보 수집 ({i}/2): {course['강의제목']}")
                    detail = self._extract_course_detail(page, course['강의링크'])
                    if detail:
                        self.data['course_details'].append(detail)
                self._log_step_complete("강의 상세 정보 수집", len(self.data['course_details']))
                ...
                self._log_progress(f"크롤링 완료! 총 소요시간: {elapsed_time:.2f}초")
                self._log_progress(f"수집 결과:")
                self._log_progress(f"   - 메인 카테고리: {len(self.data['main_categories'])}개")
                self._log_progress(f"   - 하위 카테고리: {len(self.data['sub_categories'])}개")
                self._log_progress(f"   - 강의 목록: {len(self.data['courses'])}개")
                self._log_progress(f"   - 강의 상세: {len(self.data['course_details'])}개")
